...
Ti.App.addEventListener('app:friendListAcquired', function(message) {
	...
	for (var i = 0; i < friends.length; i++) {
		checkboxArray.push({
			...
			leftImage : 'http://graph.facebook.com/' + friends[i].id + '/picture?type=small',
			accessibilityValue : friends[i].id
		});
	}
	...
	$.submit.addEventListener("touchstart", function(e) {
		$.submit.backgroundColor = "#9CC075";
	});
	$.submit.addEventListener("touchcancel", function(e) {
		$.submit.backgroundColor = "#7C9A5B";
	});
